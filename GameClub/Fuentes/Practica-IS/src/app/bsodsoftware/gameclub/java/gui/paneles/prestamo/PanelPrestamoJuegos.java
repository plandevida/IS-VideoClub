...
public class PanelPrestamoJuegos extends JPanel {
	 ...
	 * Create the panel.
	 */
	public PanelPrestamoJuegos(Sistema sistema, VentanaPrincipal ventanaPadre) {
	...
	private void init() {
		...
		panel.setLayout(new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("35dlu"),
				...
				FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC,
				FormFactory.RELATED_GAP_COLSPEC,
				ColumnSpec.decode("pref:grow"), FormFactory.GLUE_COLSPEC,
				FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC,
				FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC,
				FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC,
				FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC,
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectedRowCount() > 0) {
					...
					if (!"".equals(tDNI.getText())) {
						...
						Usuario usuario = miSistema.buscaUsuario(tDNI.getText()
								.trim());

						// Si no se ha encontrado el usuario, se muestra un
						// error.
						if (usuario != null) {

							// Generamos una fecha de devolución posterior a la
							// de prestamo.
							...
							// Se genera el prestamo con el usuario, el juego, y
							// la fecha actual, y de devolución.
							Prestamo prestamo = new Prestamo(usuario,
									juegoAPrestar, fechaPrestamo,
									fechaDevolucion);

							// Se comprueba que el préstamo se a realizado
							// corretamente.
							if (!miSistema.addPrestamo(prestamo)) {
								JOptionPane.showMessageDialog(ventana,
										"Error al crear un prestamo.", "Error",
						...
						JOptionPane
								.showMessageDialog(
										ventana,
										"Es necesario introducir el DNI (con letra y sin espacios).",
										"Aviso", JOptionPane.WARNING_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(ventana,
							"Debe seleccionar un juego.", "Aviso",
		...
		// Panel que contiene los radio buttons (si no están en un panel no se
		// intercambia entre sí la selección)
		...
		ActionListener listenerRadioButton = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// En base a que el radio button está activo, se activa el
		...
		ActionListener listenerBotonFiltrar = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Dependiendo del filtro escogido se determina la columna por
				// la que filtrar.
		...
		// Este comando sirve para determinar en que columna se va a filtrar en
		// la tabla
		...
		cNumeroJugadores.setModel(new DefaultComboBoxModel<String>(
